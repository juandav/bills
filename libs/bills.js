  ...
  if(Array.isArray(data.routes)){
    ...
    _.forEach(data.routes, function(route, key) {
      ...
      _.forEach(route.controller.method, function(method, key) {
        ...
        var feature = require('./controllers' + path)[method.feature];
        var means = method.name.toLowerCase();

        if( authenticate ){
          if( method.auth ){
            app[ means ]( path, authenticate, list.verify(method.acl), feature );
          }else{
            app[ means ]( path, list.verify(method.acl), feature );
          }   
        }else{
          app[ means ]( path, list.verify(method.acl), feature );
        }
